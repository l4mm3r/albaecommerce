...
const fetchTopSellers = async () => {
  if (topSellersCache) return topSellersCache;
  try {
    ...
    topSellersCache = response.data;
    return topSellersCache;
  } catch (error) {
    handleError(error);
  }
};
    ...
    if (positionThisProductInCart >= 0) {
      ...
      if (quantity > 0) {
        ...
        if (carts[positionThisProductInCart].quantity === 0) {
          ...
          document
            .querySelector(`.listCart [data-id="${productoId}"]`)
            .remove();
          saveCartToLocalStorage();
